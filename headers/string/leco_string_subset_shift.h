...
#ifndef PIECEWISEFIX_STRING_SUBSET_SHIFT_TEMPLATE_H_
#define PIECEWISEFIX_STRING_SUBSET_SHIFT_TEMPLATE_H_
...
namespace Codecset
{
    ...
    template <typename T>
    class Leco_string_subset_shift {
    public:
        ...
        void decodeArray8_string(uint8_t* in, const size_t length, size_t nvalue,
            ...
            if (maxbits == 0) {
                for (int i = 0; i < length; i++) {
                    ...
                    string_vec.emplace_back(convertToString_subset_shift<T>(min_ascii, max_ascii, &tmp_val));
                }
            }
            else {
    ...
    std::string randomdecodeArray_leco_string(const uint8_t* in, int idx, T* result, int min_ascii, int max_ascii) {
        ...
        std::string tmp = convertToString_subset_shift<T>(min_ascii, max_ascii, result, block_pad_length);
