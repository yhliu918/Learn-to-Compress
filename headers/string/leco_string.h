...
namespace Codecset
{
    ...
    std::string randomdecode_string(const uint8_t* in, int idx){
        const uint8_t* tmpin = in;
        ...
        std::string common_prefix(reinterpret_cast<const char*>(tmpin), common_prefix_length);
        ...
        switch(encoding_type){
            case 0:{
                ...
                result_str = convertToString<uint64_t>(&result, ori_length);
                return common_prefix + result_str;
            }
            case 1: {
                uint128_t result;
                uint8_t ori_length =randomdecodeArray_leco_string<uint128_t>(tmpin, idx, &result);
                result_str = convertToString<uint128_t>(&result, ori_length);
                return common_prefix + result_str;
            }
            case 2:{
                uint256_t result;
                uint8_t ori_length = randomdecodeArray_leco_string<uint256_t>(tmpin, idx, &result);
                result_str = convertToString<uint256_t>(&result, ori_length);
                return common_prefix + result_str;
            }
            default:
