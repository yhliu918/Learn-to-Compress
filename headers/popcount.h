...
inline uint64_t popcountLinear(uint64_t *bits, uint64_t x, uint64_t nbits) { //x is from where, nbits is goal bit
    if (nbits == 0) { return 0; }
    ...
    //std::cout<<"bits: "<<bits[x+lastword]<<" shift "<< (63 - ((nbits - 1) & popcountmask))<<std::endl;
    uint64_t lastshifted = bits[x+lastword] >> (63 - ((nbits - 1) & popcountmask));
    //std::cout<<"lastshifted: "<<lastshifted<<" pop: "<<popcount(lastshifted)<<std::endl;
