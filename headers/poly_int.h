...
namespace Codecset
{
    ...
    public:
        ...
        uint8_t *encodeArray8_int(const T *data, const size_t length, uint8_t *res, size_t nvalue)
        {
            ...
            auto simple_fixed = andviane::polynomial_regression<degree,float>(data_vec); 
        ...
        T *decodeArray8(const uint8_t *in, const size_t length, T *out, size_t nvalue)
        {
            ...
            std::array<float, degree + 1> theta;
            for (auto i = 0; i < degree + 1; i++)
            {
                memcpy(&(theta[i]), tmpin, sizeof(float));
                tmpin += sizeof(float);
            }
            andviane::Polynomial<degree,float> poly(theta);

            if (maxerror)
            {

                // read_all_bit_fix_add<T>(tmpin, 0, 0, length, maxerror, theta1, theta0, res);
                read_all_bit_fix_poly<T,degree, float>(tmpin, 0, 0, length, maxerror, res, &poly);
            }
            else
            {
        ...
        T randomdecodeArray8(const uint8_t *in, int to_find, uint32_t *out, size_t nvalue)
        {
            ...
            if (maxbits)
            {
                tmp_val = read_bit_fix_int_wo_round_poly<T,degree,float>(tmpin, maxbits, to_find, &poly);
            }
            else
            {
