...
#ifndef POLY_INTEGER_TEMPLATE_MAX_H_
#define POLY_INTEGER_TEMPLATE_MAX_H_
...
namespace Codecset
{
    ...
    double*  solve_lp(T* y_val, int number)
    {
        if(number<=degree+1){
            return 0;
        }
        ...
        double* param = new double[degree+1];
        for(int i=0;i<degree+1;i++){
            param[i] = x(i);
        }
        return param;
    ...
    public:
        ...
        uint8_t *encodeArray8_int(const T *data, const size_t length, uint8_t *res, size_t nvalue)
        {
            ...
            double* thetatmp = solve_lp<T, degree>(data_vec.data(), length);
            std::array<double, degree + 1> theta;
            for (auto i = 0; i < degree + 1; i++)
            {
                theta[i] = thetatmp[i];
            }
            andviane::Polynomial<degree> simple_fixed(theta);
