...
#ifndef PIECEWISEFIXINTOP_H_
#define PIECEWISEFIXINTOP_H_
...
class piecewise_fix_int_op : public IntegerCODEC {
public:
...
uint8_t * encodeArray8(uint32_t *in, const size_t length,uint8_t *res, size_t nvalue) {
    ...
    for(int i=0;i<(long long)length;i++){
        int tmp = in[i] - (long long)(theta0+theta1*(float)i);
        delta[i]=tmp;
...
uint32_t *decodeArray8( uint8_t *in, const size_t length, uint32_t *out, size_t nvalue) {
    ...
    tmpin++;
    memcpy(&theta0,tmpin,sizeof(int));
    tmpin+=sizeof(int);
    memcpy(&theta1,tmpin,sizeof(int));
    tmpin+=sizeof(int);
    if(maxerror>=31){
...
uint32_t randomdecodeArray8( uint8_t *in, const size_t l, uint32_t *out, size_t nvalue){
    ...
    memcpy(&maxerror,tmpin,1);
    tmpin++;
    memcpy(&theta0,tmpin,sizeof(float));
    tmpin+=sizeof(float);
    ...
    if(maxerror>=31){
       ...
       tmp = read_bit_default(tmpin ,maxerror, l, (double)theta1,(double)theta0, 0);
       //return interpret[l];
     }
    else{
       tmp = read_bit_fix_int_T(tmpin ,maxerror, l, theta1,theta0, 0);
    }
