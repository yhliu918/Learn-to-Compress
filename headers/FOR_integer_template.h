...
class FOR_int {
 public:
  ...
  int filter_range(const uint8_t* in, const size_t length, T filter, uint32_t* out,
    ...
    if (maxerror) {
      count = filter_read_all_bit_FOR<T>(tmpin, 0, length, maxerror, base, out, filter,
                                         block_start);
    } else {
      if (base > filter) {
        for (int i = 0; i < length; i++) {
  ...
  int filter_range_close(const uint8_t* in, const size_t length, uint32_t* out,
    ...
    if (maxerror) {
      count = filter_read_all_bit_FOR_close<T>(tmpin, 0, length, maxerror, base, out,
                                               block_start, filter1, filter2);
    } else {
  ...
  int filter_range_close_mod(const uint8_t* in, const size_t length, uint32_t* out,
    ...
    while (tmp_filter1 < max) {
      if (maxerror) {
        count +=
            filter_read_all_bit_FOR_close<T>(tmpin, 0, length, maxerror, base, out,
                                                 block_start, tmp_filter1, tmp_filter2);
      } else {
