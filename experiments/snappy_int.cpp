...
int main(int argc, const char *argv[])
{
    ...
    for (int i = 0; i < blocks; i++)
    {
        ...
        int segment_size = snappy::Compress((char *)(data.data() + i * block_size), block_length * sizeof(uint32_t), &compressed);
    ...
    for (int iter = 0; iter < repeat; iter++)
    {
        for (int i = 0; i < blocks; i++)
        {
            ...
            snappy::Uncompress(block_start_vec[i], seglen[i], &uncomp_str);
