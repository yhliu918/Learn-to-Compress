...
#ifdef NDEBUG
constexpr size_t REPEAT_TIMES = 3000000;
#else
constexpr size_t REPEAT_TIMES = 1;
#endif
...
int main(int argc, const char* argv[]) {
  ...
  uint8_t* out = (uint8_t*)malloc(VECTOR_LEN * sizeof(leco_type));
  //   std::vector<leco_type> recover(VECTOR_LEN);
  leco_type recover[VECTOR_LEN] = {0};

  uint8_t* out2 =
      codec.encodeArray8_int(untransposed_arr_ow32, VECTOR_LEN, out, 0);
  uint64_t cycles = cycleclock::Now();
  for (size_t i = 0; i < REPEAT_TIMES; ++i) {
    codec.decodeArray8(out, VECTOR_LEN, recover, 0);
  }
  cycles = cycleclock::Now() - cycles;
  std::cout << "Normal decode: "
            << double(cycles) / (double(REPEAT_TIMES) * VECTOR_LEN)
            << " cycles per tuple" << std::endl;
  for (size_t i = 0; i < VECTOR_LEN; ++i) {
    if (recover[i] != untransposed_arr_ow32[i]) {
      std::cout << "error" << std::endl;
    }
  }

  out2 = codec.encodeArray8_int_simd(untransposed_arr_ow32, VECTOR_LEN, out, 0);
  cycles = cycleclock::Now();
  for (size_t i = 0; i < REPEAT_TIMES; ++i) {
    codec.decodeArray8_no_branch(out, VECTOR_LEN, recover, 0);
  }
  cycles = cycleclock::Now() - cycles;
  std::cout << "No branch decode: "
            << double(cycles) / (double(REPEAT_TIMES) * VECTOR_LEN)
            << " cycles per tuple" << std::endl;
  for (size_t i = 0; i < VECTOR_LEN; ++i) {
    if (recover[i] != untransposed_arr_ow32[i]) {
  ...
  out2 =
      codec.encodeArray8_int_nopack(untransposed_arr_ow32, VECTOR_LEN, out, 0);
  cycles = cycleclock::Now();
  for (size_t i = 0; i < REPEAT_TIMES; ++i) {
    codec.decodeArray8_no_pack(out, VECTOR_LEN, recover, 0);
  }
  cycles = cycleclock::Now() - cycles;
  std::cout << "AVX512 no pack decode: "
            << double(cycles) / (double(REPEAT_TIMES) * VECTOR_LEN)
            << " cycles per tuple" << std::endl;
  for (size_t i = 0; i < VECTOR_LEN; ++i) {
    if (recover[i] != untransposed_arr_ow32[i]) {
...
//         base = atoll(argv[7]);
//     }
//     // alternatives : Delta_int, Delta_cost, Delta_cost_merge, FOR_int,
//     Leco_int, Leco_cost, Leco_cost_merge_hc,  Leco_cost_merge,
//     Leco_cost_merge_double
...
//         uint8_t* descriptor = (uint8_t*)malloc(block_length *
//         sizeof(leco_type) * 4); uint8_t* res = descriptor; res =
//         codec.encodeArray8_int(data.data() + (i * block_size), block_length,
//         descriptor, i); uint32_t segment_size = res - descriptor; descriptor
//         = (uint8_t*)realloc(descriptor, segment_size);
...
//     double cr_wo_model = (totalsize - total_model_size) * 100.0 /
//     origin_size; double cr_model = total_model_size * 100.0 / origin_size;
...
//                 block_length = N - (blocks - 1) * block_size;
//             }
//             codec.decodeArray8(block_start_vec[i], block_length,
//             recover.data() + i * block_size, i);
//         }
...
//                 predict is: " << recover[j] << std::endl; std::cout <<
//                 "something wrong! decompress all failed" << std::endl; flag =
//                 false; break;
//             }
//         }
//         if (!flag)
...
//         leco_type tmpvalue =
//         codec.randomdecodeArray8(block_start_vec[(int)index / block_size],
//         index % block_size, NULL, N); mark += tmpvalue; #ifndef NDEBUG if
//         (data[index] != tmpvalue)
//         {

//             std::cout << "num: " << index << "true is: " << data[index] << "
//             predict is: " << tmpvalue << std::endl; flag = false; std::cout
//             << "something wrong! random access failed" << std::endl;
//         }
//         if (!flag)
//         {
//             break;
...
//                     block_length = N - (blocks - 1) * block_size;
//                 }
//                 total_counter += codec.filter_range(block_start_vec[i],
//                 block_length, filter1, bit_pos.data() + total_counter, i);
//             }
//         }
//         end = getNow();
//     }
...
//             {
//                 // if(zonemap[i].second<=filter1 ||
//                 zonemap[i].first>=filter2){
//                 //     continue;
//                 // }
//                 leco_type tmp_filter1 = ceil((double)(zonemap[i].first -
//                 filter1)/(double)base)*base + filter1; leco_type tmp_filter2
//                 = ceil((double)(zonemap[i].first -
//                 filter1)/(double)base)*base + filter2;
//                 if(zonemap[i].first%base < filter2 && zonemap[i].first%base>
//                 filter1){
//                     tmp_filter1-=base;
//                     tmp_filter2-=base;
//                 }
...
//                     block_length = N - (blocks - 1) * block_size;
//                 }
//                 int tmpcount= codec.filter_range_close(block_start_vec[i],
//                 block_length, bit_pos.data() + total_counter, i, tmp_filter1,
//                 tmp_filter2, base); total_counter+=tmpcount;

//                 int truecount = 0;
//                 for(int j = 0;j<block_length;j++){
//                     if(data[i*block_size + j]%base>filter1
//                     &&data[i*block_size + j]%base<filter2){
//                         truecount++;
//                     }
//                 }
//                 if(truecount!=tmpcount){
//                     std::cout<<"wrong"<<i<<" "<<truecount<<"
//                     "<<tmpcount<<std::endl;
//                 }
//             }
//         }
...
//     compressrate << " " << cr_model << " " << cr_wo_model << " " << da_ns <<
//     " " << ra_ns<<" "<<0<<" "<<total_counter<<" "<<filter_totaltime<<"
//     "<<filter_close_totaltime<< std::endl;
