...
int main(int argc, const char* argv[])
{
    ...
    // Leco_int_poly<leco_type, 3> codec_degree3;
    // codec_degree3.init(blocks, block_size);
    ...
    for (int i = 0; i < blocks; i++)
    {
        ...
        uint32_t segment_size_degree2 = res - descriptor;

        // res = codec_degree3.encodeArray8_int(data.data() + (i * block_size), block_length, descriptor, i);
        // uint32_t segment_size_degree3 = res - descriptor;

        int poly_degree = 1;
        if (segment_size_degree2 < segment_size)
        {
            poly_degree = 2;
            // if(segment_size_degree3 < segment_size_degree2){
            //     poly_degree = 3;
            // }
        }
