...
int main() {
  ...
  // We pick a CODEC
  IntegerCODEC &codec = *CODECFactory::getFromName("piecewise_fix_32");
  ...
  for(int i=0;i<blocks;i++){
    uint32_t * descriptor = (uint32_t*)malloc(block_size * sizeof(uint64_t));
    uint32_t * res = descriptor;
    res = codec.encodeArray(data.data()+(i*block_size),block_size ,descriptor,i);
