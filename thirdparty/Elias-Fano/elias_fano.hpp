    ...
    public:
        ...
        struct elias_fano_builder {
            elias_fano_builder(uint64_t n, uint64_t m)
                : m_n(n) // max element
                , m_m(m) // number of elements
                , m_pos(0)
                , m_last(0)
                , m_l(uint8_t((m && n / m) ? broadword::msb(n / m) : 0)) // low bits per element
            ...
            inline void push_back(uint64_t i) {
                ...
                if (m_l) {
                    // std::cout<<(i & low_mask) <<" "<<i<<" "<<m_l<<std::endl;
        ...
        inline uint64_t operator[](uint64_t pos) const {
            assert(pos < size());
            assert(m_high_bits_d0.num_positions()); // needs rank index
            uint64_t h_pos = m_high_bits_d0.select(m_high_bits, pos);
            std::cout<<pos<<" "<<h_pos<<std::endl;
            uint64_t high = h_pos - pos + 1;
            high = high << m_l;
            uint64_t low = m_low_bits.get_bits(pos * m_l, m_l);
            return high | low;
